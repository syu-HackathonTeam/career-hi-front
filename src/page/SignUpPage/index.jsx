...
import { api_signup, api_signupCertification } from "../../api/signup";
...
const SignUpPage = () => {
  ...
  const onClickSignUp = useCallback(async () => {
    ...
    try {
      ...
      if (!res?.success) {
        setAlertTitleText(res?.message || "회원가입에 실패했습니다");
      ...
      setLoginInfo({
        isLogin: true,
        userData: {
          userId: res.userInfo?.userId ?? null,
          userName: res.userInfo?.userName ?? null,
        },
        // 마지막 검증 시간 (타임스탬프)
      ...
      // 토큰 저장
      if (res.tokenInfo?.accessToken) {
        localStorage.setItem("jwt_token", res.tokenInfo.accessToken);
      }
      if (res.tokenInfo?.refreshToken) {
